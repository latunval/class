...
for (let i = 0; i < employers.length; i++) {
  const element = `${employers[i].name} ${employers[i].position} $${employers[
    i
  ].getAnnualSalary()}`;
...
for (let i = 0; i < users.length; i++) {
  const element = ` ${users[i].deposit(50)}, ${users[i].withdraw(100)} ${users[
    i
  ].getBalance()}`;
  console.log(element);
}
...
class Person {
  constructor(name, age) {
    (this.owner = name), (this.balance = age);
  }
  getDetails() {
...
class Student extends Person {
  ...
  getStudentInfo() {
    return `  ${this.getDetails()} and has study this${
      this.course
    } which has this${this.grade}`;
  }
}
...
class Order {
  constructor(id, name) {
    (this.orderid = id), (this.name = name);
  }
  getDetails() {
...
class ProductOrder extends Order {
  ...
  getOrderDetails() {
    return `${this.getDetails()} bought this ${
      this.productName
    } at the rate of ${this.price}`;
  }
}
...
class Patient extends Persons {
  constructor(name, age, illness, hospitalName) {
    super(name, age);
    (this.course = illness), (this.grade = hospitalName);
  }
  getPatientDetails() {
    return `  ${this.getDetails()} and has study this${
      this.course
    } which has this${this.grade} hospital`;
  }
}
...
class MovieTicket extends Movie {
  constructor(title, duration, seatNumber, price) {
    super(title, duration), (this.seat = seatNumber), (this.price = price);
  }
  getTicketInfo() {
    return `${this.getMovieDetails()} for number ${
      this.seat
    } with the price of ${this.price}`;
  }
}
...
class Course {
  constructor(courseName, instructor) {
    (this.name = courseName), (this.instructor = instructor);
  }
  getDetails() {
...
class StudentCourse extends Course {
  constructor(courseName, instructor, progress) {
    super(courseName, instructor), (this.progress = progress);
  }
  details() {
    return `${this.getDetails()} its progress to another level ${
      this.progress
    }`;
  }
}
...
class Ride extends Vehicles {
  ...
  details() {
    return `${this.getVehicleDetails()} and the driver name  is ${
      this.driverName
    } that is giving tihs passenger ${this.passenger} rides at the rate of ${
      this.fare
    }`;
  }
}
